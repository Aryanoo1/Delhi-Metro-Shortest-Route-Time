...
public class Metro_Graph {
	...
	public static HashMap<Character, String> color = new HashMap<>();

	public static void add_lines() {
		color.put('R', "RED");
		color.put('B', "BLUE");
		color.put('P', "PINK");
		color.put('Y', "YELLOW");
	...
	public static void station_names(Metro_Graph graph) {
	...
	public static void stations_forUsers() {
		String[] stations = { "DWARKA SECTOR 21", "DWARKA", "JANAKPURI WEST", "RAJOURI GARDEN", "KIRTI NAGAR",
				"RAJIV CHOWK", "MANDI HOUSE", "YAMUNA BANK", "KARKARDOOMA", "VAISHALI", "MAYUR VIHAR",
				"BOTANICAL GARDEN", "NOIDA ELECTRONIC CITY", "NEW DELHI", "DHANSA BUS STAND", "HAUZ KHAS",
				"KALKAJI MANDIR", "MAJLIS PARK", "AZADPUR", "NETAJI SUBHASH PLACE", "PUNJABI BAGH WEST", "INA",
				"LAJPAT NAGAR", "SHIV VIHAR", "BAHADURGARH CITY PARK", "ASHOK PARK", "INDERLOK", "SAMAYPUR BADLI",
				"KASHMERE GATE", "CENTRAL SECRETARIAT", "HUDA CITY CENTER", "ESCORTS MUJESAR", "RITHALA",
				"SHAHEED STHAL" };
	...
	public static void create_edges(Metro_Graph graph) {
	...
	public static ArrayList<String> interchanges(ArrayList<String> list) {
		...
		for (int j = 1; j < code_string.size() - 1; j++) {
			...
			if (line1 != line2) {
				list.add(code_string.size() - j - 1, "(Change to " + color.get(line2) + " Line)");
				counter++;
			}
		}
	...
	public static boolean station_check(String s1, String s2) {
		...
		for (Map.Entry m : vertices.entrySet()) {
			String[] arr = m.getKey().toString().split("~");
			map.put(arr[0], (branch) m.getValue());
		}
	...
	public static void Input3(BufferedReader scn, Metro_Graph graph) throws IOException {
		stations_forUsers();
		System.out.println();
		System.out.println();
	...
	public static void Input4(BufferedReader scn, Metro_Graph graph) throws IOException {
		stations_forUsers();
		System.out.println();
		System.out.println();
		System.out.print("Enter START STATION : ");
		...
		if (station_check(s1, s2)) {
			...
			System.out.println();
			System.out.println("The Number of interchanges will be : " + list.get(list.size() - 1));
		} else {
			System.out.println();
			System.out.println("Invalid Station Entries!!");
			System.out.println("Try Again!!");
	...
	public static void start() throws IOException {
		...
		Metro_Graph graph = new Metro_Graph();
		station_names(graph);
		create_edges(graph);
		add_lines();
		main();
